...
public:
	...
	enum EXformId
	{
		...
		ExfExpandNAryJoin____removed,
		ExfExpandNAryJoinMinCard____removed,
		ExfExpandNAryJoinDP____removed,
		ExfGet2TableScan,
		...
		ExfGbAgg2Apply,
		ExfSubqJoin2Apply,
		ExfSubqNAryJoin2Apply____removed,
		...
		ExfPushDownLeftOuterJoin____removed,
		...
		ExfExpandNAryJoinGreedy____removed,
		ExfEagerAgg,
		ExfExpandNAryJoinDPv2____removed,
	...
	FDSLShell(EXformId exfid)
	{
		return (ExfDSLRuleSelect <= exfid && exfid <= ExfDSLRuleLimit) ||
			...
			(ExfDSLRuleIntersect <= exfid &&
			 exfid <= ExfDSLRuleCTEConsumer);
	}
