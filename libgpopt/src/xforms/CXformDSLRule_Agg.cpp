...
//
//	@doc:
//		Ctor — one complete-tree pattern shared by the two ORCA aggregate shells.
//		Registration limits this xform to LogicalGbAgg and
//		LogicalGbAggDeduplicate; CPatternTree materializes either complete tree so
//		the shared matcher can inspect the relational and scalar children.
//---------------------------------------------------------------------------
...
CXformDSLRule_Agg::Exfp(CExpressionHandle &exprhdl) const
{
	...
	const COperator::EOperatorId eopid = exprhdl.Pop()->Eopid();
	...
	if (nullptr == peng ||
		0 == peng->PdrgpruleForRoot(eopid)->Size())
	{
		return CXform::ExfpNone;
	}
