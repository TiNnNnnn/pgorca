...
FROM EnhancedUserStats AS up
JOIN UserRanks AS ur
  ON up.UserId = ur.UserId
INNER JOIN RankedPosts AS iron
  ON up.UserId = iron.OwnerUserId AND iron.Rank <= 5
LEFT JOIN PostHistorySummary AS phs
  ON iron.Id = phs.PostId
WHERE
  (
  ...
  AND (
    phs.HistoryCount > 1 OR NOT phs.LastModifiedDate IS NULL
  )
ORDER BY
  up.TotalBounty DESC,
