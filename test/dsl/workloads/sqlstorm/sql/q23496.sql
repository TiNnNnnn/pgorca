...
SELECT
  ...
  CASE
    WHEN COALESCE(ubs.TotalBadges, 0) > 0
    THEN 'Active User'
    WHEN COALESCE(ps.TotalPosts, 0) = 0 AND COALESCE(vcs.VotedPosts, 0) = 0
    THEN 'Inactive User'
    ELSE 'Moderate User'
  END AS UserStatus,
...
LEFT JOIN UserBadgeStats AS ubs
  ON u.Id = ubs.UserId
LEFT JOIN PostSummary AS ps
  ON u.Id = ps.OwnerUserId
LEFT JOIN VoteCounts AS vcs
  ON u.Id = vcs.UserId
WHERE
  u.Reputation > 1000
  AND (
    COALESCE(ubs.TotalBadges, 0) > 2 OR COALESCE(ps.TotalPosts, 0) > 5
  )
ORDER BY
  Rank,
  u.DisplayName
