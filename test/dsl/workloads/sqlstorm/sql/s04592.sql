...
SELECT
  ...
  pp.ViewCount,
  pp.CommentCount,
  CASE WHEN us.Location IS NULL THEN 'Unknown Location' ELSE us.Location END AS UserLocation,
  pht.Name AS PostHistoryType
FROM UserStatistics AS us
LEFT JOIN Posts AS p
  ON us.UserId = p.OwnerUserId
LEFT JOIN TopPosts AS pp
  ON p.Id = pp.PostId
...
WHERE
  ...
  AND (
    pp.ScoreRank <= 10 OR pp.ViewCount > 100
  )
  AND pp.CommentCount > COALESCE((
    SELECT
      AVG(CommentCount)
    FROM PostDetails
  ), 0)
