WITH RankedPosts AS (
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'Closure Activity'
      WHEN ph.PostHistoryTypeId IN (6, 4)
      THEN 'Tag or Title Activity'
      ELSE 'Other Activity'
    END AS ActivityType,
...
SELECT
  ...
  pa.UniqueUserCount,
  CASE
    WHEN pa.UniqueUserCount > 5
    THEN 'Highly Active'
    WHEN pa.UniqueUserCount BETWEEN 3 AND 5
    THEN 'Moderately Active'
    ELSE 'Low Activity'
  END AS ActivityLevel
FROM RankedPosts AS rp
LEFT JOIN UserBadges AS ub
  ON rp.OwnerUserId = ub.UserId
LEFT JOIN PostActivity AS pa
  ON rp.PostId = pa.PostId
WHERE
  rp.UserPostRank <= 3 AND (
    NOT pa.ActivityType IS NULL OR ub.BadgeCount > 0
  )
ORDER BY
  rp.Score DESC,
  rp.CreationDate ASC;
