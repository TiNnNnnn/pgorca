...
SELECT
  ...
  CASE WHEN us.Reputation >= 1000 THEN 'Highly Reputable' ELSE 'Newcomer' END AS UserStatus
FROM UserStats AS us
LEFT JOIN RecentPosts AS rp
  ON us.UserId = rp.OwnerUserId AND rp.RecentPostRank = 1
LEFT JOIN TopPosts AS tp
  ON us.UserId = tp.OwnerUserId AND tp.PostRank = 1
WHERE
  us.UserRank <= 10
ORDER BY
  us.Reputation DESC;
