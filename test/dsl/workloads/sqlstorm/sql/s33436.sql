...
LEFT JOIN UserBadgeCounts AS ubc
  ON u.Id = ubc.UserId
INNER JOIN RecentPosts AS rp
  ON u.Id = rp.OwnerUserId AND rp.RecentPostRank = 1
LEFT JOIN VoteSummary AS vs
  ON rp.PostId = vs.PostId
LEFT JOIN CommentsWithDetails AS cd
  ON rp.PostId = cd.PostId AND cd.CommentRank = 1
WHERE
  u.Reputation > 1000
ORDER BY
  ubc.BadgeCount DESC,
