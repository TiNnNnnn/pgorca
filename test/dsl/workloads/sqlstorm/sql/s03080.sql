WITH RankedBadges AS (
  ...
  LEFT JOIN Votes AS v
    ON p.Id = v.PostId
  LEFT JOIN PostHistory AS bh
    ON p.Id = bh.PostId AND bh.PostHistoryTypeId IN (4, 5)
  WHERE
  ...
  FROM Users AS u
  LEFT JOIN RankedBadges AS b
    ON u.Id = b.UserId AND b.BadgeRank = 1
  LEFT JOIN PostInformation AS pi
...
SELECT
  ...
  us.TotalUpVotes,
  us.TotalDownVotes,
  CASE WHEN us.PostCount > 10 THEN 'Active' ELSE 'Less Active' END AS ActivityStatus,
