...
SELECT
  ...
  c.AvgViewCount,
  CASE
    WHEN NOT c.LastPostDate IS NULL
    THEN CAST('2024-10-01' AS DATE) - c.LastPostDate
    ELSE NULL
  END AS DaysSinceLastPost
FROM CombinedMetrics AS c
WHERE
  (
    c.TotalQuestions > 0 OR c.TotalAnswers > 0
  )
  AND (
    c.GoldBadgeCount > 0 OR c.SilverBadgeCount > 0 OR c.BronzeBadgeCount > 0
  )
ORDER BY
  c.TotalScore DESC,
  c.TotalPosts DESC
