WITH RankedPosts AS (
  ...
  WHERE
    p.CreationDate >= (
      CAST('2024-10-01' AS DATE) - INTERVAL '1 YEAR'
    )
), PostVotes AS (
  SELECT
    v.PostId,
...
SELECT
  ...
  fp.UpVotes,
  fp.DownVotes,
  CASE WHEN fp.LastClosedDate = '1970-01-01' THEN NULL ELSE fp.LastClosedDate END AS ClosedDate,
  CASE WHEN fp.LastReopenedDate = '1970-01-01' THEN NULL ELSE fp.LastReopenedDate END AS ReopenedDate
FROM FinalPosts AS fp
WHERE
