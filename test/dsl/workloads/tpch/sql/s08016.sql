...
FROM RankedCustomers AS R
LEFT JOIN HighValueOrders AS HVO
  ON HVO.nation_name = R.nation_name
LEFT JOIN SupplierPartInfo AS S
  ON S.s_suppkey = R.c_custkey
LEFT JOIN partsupp AS PA
  ON PA.ps_partkey = R.c_custkey
WHERE
  R.rank_per_nation <= 5
GROUP BY
  r.nation_name
