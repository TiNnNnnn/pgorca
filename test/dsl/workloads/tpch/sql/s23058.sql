WITH RankedSuppliers AS (
  ...
  JOIN nation AS ns
    ON rs.s_suppkey = ns.n_nationkey
  JOIN HighValueParts AS hp
    ON hp.total_parts_value > 0
  ...
  LEFT JOIN lineitem AS l
    ON od.o_orderkey = l.l_orderkey AND l.l_returnflag = 'R'
  WHERE
    rs.rank = 1
)
SELECT
  ...
  fr.order_total,
  CASE
    WHEN fr.order_rank IS NULL
    THEN 'No Rank Available'
    ELSE CAST(fr.order_rank AS VARCHAR)
  END AS order_rank
FROM FinalReport AS fr
WHERE
  NOT fr.order_total IS NULL
ORDER BY
  fr.nation_name,
  fr.order_total DESC;
