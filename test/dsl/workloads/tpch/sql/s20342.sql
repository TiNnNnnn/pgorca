WITH RankedOrders AS (
  ...
  SELECT
    ...
    CASE WHEN ss.part_count > 5 THEN 'High Supplier' ELSE 'Low Supplier' END AS supplier_type
  FROM supplier AS s
  ...
  LEFT JOIN lineitem AS l
    ON ro.o_orderkey = l.l_orderkey
  LEFT JOIN FilteredSuppliers AS fs
    ON l.l_suppkey = fs.s_suppkey
  GROUP BY
    ro.o_orderkey,
    ro.o_totalprice,
    ro.o_orderdate,
