...
SELECT
  ...
  CASE WHEN SUM(l.l_discount) > 0 THEN 'Discount Applied' ELSE 'No Discount' END AS discount_status
FROM RankedOrders AS rp
LEFT JOIN lineitem AS l
  ON rp.o_orderkey = l.l_orderkey
JOIN CustomerOrders AS co
  ON rp.o_custkey = co.c_custkey
LEFT JOIN SupplierParts AS sp
  ON l.l_partkey = sp.p_partkey AND sp.rn = 1
WHERE
  rp.rn = 1
GROUP BY
  co.c_name,
