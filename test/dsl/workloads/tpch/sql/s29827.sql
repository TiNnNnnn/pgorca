WITH RECURSIVE string_benchmark AS (
  SELECT
    ...
    'Part: ' || p.p_name || ' | Size: ' || p.p_size || ' | Price: ' || CAST(p.p_retailprice AS VARCHAR) AS formatted_info
  FROM part AS p
  WHERE
    p.p_size > 10
  UNION ALL
  SELECT
    ...
    'Supplier: ' || s.s_name || ' | Address: ' || s.s_address || ' | Phone: ' || s.s_phone AS formatted_info
  FROM supplier AS s
  JOIN nation AS n
