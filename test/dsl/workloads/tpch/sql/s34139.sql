...
JOIN NationDetails AS nd
  ON sc.s_nationkey = nd.n_nationkey
LEFT JOIN CustomerOrders AS co
  ON sc.s_suppkey = co.c_custkey
LEFT JOIN OrderLineItems AS oli
  ON co.o_orderkey = oli.o_orderkey
WHERE
  sc.rn = 1
  AND sc.ps_supplycost < (
    SELECT
      AVG(ps_supplycost)
    FROM partsupp
  )
