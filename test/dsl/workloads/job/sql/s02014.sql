WITH RankedTitles AS (
  ...
  SELECT
    ...
    CASE WHEN m.production_year >= 2000 THEN 'Modern' ELSE 'Classic' END AS era
  FROM aka_title AS m
...
SELECT
  ...
  sa.movie_count,
  rt.rank
FROM RecentMovies AS rm
JOIN complete_cast AS cc
  ON rm.movie_id = cc.movie_id
JOIN SelectedActors AS sa
  ON cc.subject_id = sa.person_id
JOIN RankedTitles AS rt
  ON rm.title = rt.title
WHERE
  rm.era = 'Modern' AND NOT sa.name IS NULL
ORDER BY
