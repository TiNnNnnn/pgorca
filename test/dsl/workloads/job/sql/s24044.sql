WITH RankedMovies AS (
  ...
  SELECT
    ...
    SUM(CASE WHEN NOT c.note IS NULL THEN 1 ELSE 0 END) AS has_note_count
  FROM cast_info AS c
  GROUP BY
    c.movie_id
), MovieKeywords AS (
  ...
  SELECT
    ...
    rm.production_count,
    CASE WHEN rm.production_count > 5 THEN 'Popular Year' ELSE 'Less Popular Year' END AS popularity_category
  ...
  FROM MoviesWithDetails AS mwd
  WHERE
    mwd.total_actors > 3
    AND mwd.production_year BETWEEN 2000 AND 2023
    AND NOT (
      mwd.keywords IS NULL OR mwd.keywords = 'No Keywords'
    )
)
SELECT
  title,
