WITH RECURSIVE movie_actors AS (
  ...
  FROM aka_title AS ht
  LEFT JOIN high_role_movies AS ha
    ON ht.id = ha.movie_id
  GROUP BY
    ht.title,
    ht.production_year
...
SELECT
  ...
  yp.latest_movie,
  CASE
    WHEN yp.movie_count IS NULL
    THEN 'No movies produced'
    WHEN yp.movie_count > 10
    THEN 'High Production Year'
