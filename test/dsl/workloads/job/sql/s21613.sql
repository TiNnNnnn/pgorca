WITH ranked_movies AS (
  ...
  WHERE
    NOT mt.production_year IS NULL
), cast_details AS (
  SELECT
    c.movie_id,
    c.person_id,
  ...
  FROM cast_info AS c
  INNER JOIN aka_name AS ak
    ON c.person_id = ak.person_id
  LEFT JOIN role_type AS rc
    ON c.role_id = rc.id
  GROUP BY
    c.movie_id,
    c.person_id,
  ...
  SELECT
    ...
    mk.keywords,
    CASE
      WHEN rm.title_rank <= 10
      THEN 'Top Ten'
      WHEN rm.total_movies > 0 AND rm.title_rank > 10
      THEN 'Non-Top Ten'
      ELSE 'Unknown Rank'
...
FROM selected_movies AS sm
WHERE
  (
    sm.production_year BETWEEN 2000 AND 2023
  )
  AND (
    sm.movie_rank_category = 'Top Ten' OR sm.keywords LIKE '%Action%'
  )
ORDER BY
