WITH ranked_movies AS (
  ...
  SELECT
    ...
    CASE WHEN COUNT(c.person_id) > 10 THEN 'Large Cast' ELSE 'Small Cast' END AS cast_size_category
  FROM ranked_movies AS r
  LEFT JOIN movie_keyword_info AS k
    ON r.movie_id = k.movie_id
  LEFT JOIN multi_word_titles AS rw
    ON r.movie_id = rw.movie_id
  LEFT JOIN cast_info AS c
...
SELECT
  ...
  cast_size,
  CASE
    WHEN word_count IS NULL
    THEN 'Unknown Word Count'
