WITH RankedMovies AS (
  SELECT
    ...
    SUM(CASE WHEN NOT ci.note IS NULL THEN 1 ELSE 0 END) AS has_notes
  FROM aka_title AS t
  ...
  SELECT
    ...
    rm.has_notes,
    CASE
      WHEN rm.total_cast_members > 5 AND rm.has_notes > 0
      THEN 'Highly Casted with Notes'
      WHEN rm.total_cast_members > 5
      THEN 'Highly Casted'
      WHEN rm.has_notes > 0
      THEN 'Few Casted with Notes'
      ELSE 'Few Casted'
...
SELECT
  ...
  CASE
    WHEN mw.rank_by_keywords < 5
    THEN 'Consider Watching'
    WHEN mw.rank_by_keywords BETWEEN 5 AND 10
    THEN 'Mainstream Pick'
    ELSE 'Top Choice!'
  END AS recommendation
FROM MoviesWithNotes AS mw
LEFT JOIN DistinctCompanies AS dc
  ON mw.title_id = dc.movie_id
WHERE
  NOT mw.production_year IS NULL
