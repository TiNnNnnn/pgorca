WITH RECURSIVE top_movie_titles AS (
  ...
  FROM movie_companies AS c
  JOIN company_name AS cn
    ON cn.id = c.company_id
...
LEFT JOIN movie_keywords AS mk
  ON mk.movie_id = ft.title_id AND mk.keyword_rank = 1
LEFT JOIN movie_company_data AS mcd
  ON mcd.movie_id = ft.title_id AND mcd.company_rank = 1
WHERE
