...
SELECT
  ...
  CASE
    WHEN ta.movie_count > 5
    THEN 'Veteran Actor'
    WHEN ta.movie_count BETWEEN 2 AND 5
    THEN 'Emerging Actor'
    ELSE 'New Talent'
  END AS actor_category
FROM RankedTitles AS r
LEFT JOIN ActorMovies AS am
  ON r.title = am.movie_title AND r.production_year = am.production_year
LEFT JOIN TopActors AS ta
  ON am.actor_name = ta.actor_name
WHERE
  r.rank <= 10
ORDER BY
