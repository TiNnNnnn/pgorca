WITH RankedMovies AS (
  SELECT
    ...
    a.id
  FROM aka_title AS a
  LEFT JOIN movie_link AS ml
    ON a.id = ml.movie_id
  LEFT JOIN aka_title AS m
    ON ml.linked_movie_id = m.id
), CastStats AS (
  SELECT
    c.movie_id,
  ...
  LEFT JOIN company_name AS cp
    ON cp.id = mc.company_id
  GROUP BY
    mc.movie_id
)
SELECT
  ...
  ci.company_names,
  CASE
    WHEN cs.highest_order IS NULL
    THEN 'No cast members'
    ELSE 'Has cast members'
  END AS cast_status,
  COALESCE(rm.linked_movie_title, 'N/A') AS linked_movie
FROM RankedMovies AS rm
LEFT JOIN CastStats AS cs
  ON rm.id = cs.movie_id
