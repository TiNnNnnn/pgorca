...
LEFT JOIN AddressInfo AS ai
  ON ci.c_customer_sk = ai.ca_address_sk
LEFT JOIN RankedSales AS r
  ON ci.c_customer_sk = r.ws_item_sk
FULL OUTER JOIN SalesAggregate AS sa
  ON r.ws_item_sk = sa.ws_item_sk
WHERE
  ai.city_rank < 10 AND (
    ci.cd_marital_status = 'S' OR ci.cd_gender IS NULL
  )
GROUP BY
  ci.c_first_name,
  ci.c_last_name,
  ...
  ai.ca_country
HAVING
  SUM(r.ws_net_profit) > 0
  AND (
    NOT MAX(r.total_net_paid) IS NULL OR COALESCE(SUM(sa.total_quantity), 0) < 100
  )
ORDER BY
  total_profit DESC;
