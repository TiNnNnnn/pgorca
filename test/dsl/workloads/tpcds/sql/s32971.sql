WITH RECURSIVE SalesCTE AS (
  ...
  FROM web_sales
  WHERE
    ws_sold_date_sk BETWEEN 2458849 AND 2458879
  GROUP BY
    ws_item_sk
  UNION ALL
  SELECT
  ...
  JOIN SalesCTE AS cte
    ON s.ss_item_sk = cte.ws_item_sk
  WHERE
    s.ss_sold_date_sk BETWEEN 2458849 AND 2458879
  GROUP BY
    s.ss_item_sk
  ...
  FROM SalesCTE AS cte
  LEFT JOIN TotalCustomerReturns AS tr
    ON cte.ws_item_sk = tr.sr_item_sk
)
SELECT
  i.i_item_id,
  i.i_item_desc,
  ...
  CASE
    WHEN rs.total_sales > 1000
    THEN 'High'
    WHEN rs.total_sales > 500
    THEN 'Medium'
    ELSE 'Low'
