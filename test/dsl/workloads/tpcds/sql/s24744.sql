WITH RECURSIVE CustomerReturns AS (
  ...
  FROM store_returns
  WHERE
    NOT sr_return_quantity IS NULL
), AggregateReturns AS (
  SELECT
    sr_customer_sk,
  ...
  SELECT
    ...
    ar.avg_return_amt
  FROM customer AS c
  LEFT JOIN AggregateReturns AS ar
    ON c.c_customer_sk = ar.sr_customer_sk
  LEFT JOIN CustomerDemographics AS cd
  ...
  SELECT
    ...
    AVG(properties.total_returned) AS avg_returned,
    COUNT(properties.total_returned) AS total_return_count
  FROM CustomerDetails AS cd
  LEFT JOIN AggregateReturns AS properties
    ON cd.c_customer_sk = properties.sr_customer_sk
  GROUP BY
    cd.customer_value
)
SELECT
...
FROM ReturnSummary AS r
FULL OUTER JOIN (
  SELECT
    COUNT(DISTINCT s_store_sk) AS active_store_count
  FROM ActiveStores
) AS c
  ON 1 = 1
WHERE
  r.customer_count IS NULL OR r.customer_count > 10
ORDER BY
  r.customer_value DESC NULLS LAST;
