...
SELECT
  ...
  CASE WHEN SUM(tc.total_profit) IS NULL THEN 0 ELSE SUM(tc.total_profit) END AS adjusted_profit
FROM TopCustomers AS tc
LEFT JOIN ItemCategory AS ic
  ON tc.ws_bill_customer_sk = ic.i_item_sk
GROUP BY
  tc.ws_bill_customer_sk,
  ic.i_category
ORDER BY
