WITH RECURSIVE CustomerReturns AS (
  ...
  LEFT JOIN CustomerReturns AS cr
    ON c.c_customer_sk = cr.sr_customer_sk
  GROUP BY
    c.c_customer_sk,
    c.c_first_name,
    c.c_last_name
  ...
  SELECT
    ...
    (
      COALESCE(sales.total_sales, 0) - cus.total_return_value
    ) AS net_total
  FROM CustomerReturnSummary AS cus
  LEFT JOIN SalesSummary AS sales
    ON cus.c_customer_sk = sales.ws_bill_customer_sk
)
SELECT
  f.c_customer_sk,
  f.c_first_name,
