WITH CustomerReturns AS (
  ...
  SELECT
    ...
    CASE
      WHEN COALESCE(ws.total_sales, 0) = 0
      THEN NULL
  ...
  LEFT JOIN WebSalesAnalysis AS ws
...
SELECT
  ...
  CASE
    WHEN a.return_to_sales_ratio IS NULL OR a.return_to_sales_ratio > 0.5
    THEN 'High Return'
    WHEN a.return_to_sales_ratio <= 0.5 AND a.return_to_sales_ratio > 0
    THEN 'Moderate Return'
    ELSE 'No Returns'
