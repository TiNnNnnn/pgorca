WITH CustomerSaleSummary AS (
  ...
  FROM CustomerSaleSummary AS css
  WHERE
    css.total_profit > (
      SELECT
        AVG(total_profit)
  ...
  LEFT JOIN StoreSalesDetails AS ss
    ON hvc.c_customer_sk = ss.ss_customer_sk
)
SELECT
  cb.c_customer_sk,
  ...
  CASE
    WHEN cb.combined_total_profit > 1000
    THEN 'Gold'
    WHEN cb.combined_total_profit BETWEEN 500 AND 1000
    THEN 'Silver'
    ELSE 'Bronze'
  END AS customer_level
FROM CombinedSales AS cb
ORDER BY
