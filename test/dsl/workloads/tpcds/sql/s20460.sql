...
FROM demographic_analysis AS da
JOIN sales_performance AS sa
  ON da.customer_count > 10
LEFT JOIN customer_addresses AS ca
  ON ca.customers_in_address > 5
WHERE
  EXISTS(
    SELECT
      1
    FROM customer AS c
    WHERE
      NOT c.c_current_addr_sk IS NULL AND c.c_current_addr_sk = ca.ca_address_sk
  )
ORDER BY
