WITH RankedSales AS (
  SELECT
    ...
    ws_ext_tax,
    (
      CASE WHEN ws_net_profit IS NULL THEN 0 ELSE ws_net_profit END
    ) AS adjusted_profit
  FROM web_sales
  WHERE
    ws_net_profit > (
...
SELECT
  ...
  CASE
    WHEN TPI.total_paid > 10000
    THEN 'High Value'
    WHEN TPI.total_paid > 5000
    THEN 'Medium Value'
    ELSE 'Low Value'
  END AS value_segment
FROM TopProfitableItems AS TPI
ORDER BY
  TPI.max_profit DESC,
