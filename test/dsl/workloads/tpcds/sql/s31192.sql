WITH RECURSIVE sales_summary AS (
  ...
  JOIN sales_summary AS ss
    ON w.w_warehouse_sk = ss.ss_store_sk
  LEFT JOIN top_customers AS t
    ON ss.sales_rank = 1
), final_summary AS (
  SELECT
    sp.w_warehouse_id,
    ...
    CASE
      WHEN sp.total_sales > 10000
      THEN 'High Performer'
      WHEN sp.total_sales BETWEEN 5000 AND 10000
