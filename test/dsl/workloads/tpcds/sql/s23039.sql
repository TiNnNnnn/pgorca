WITH RankedSales AS (
  ...
  WHERE
    ws.ws_net_paid > 0
    AND (
      ws.ws_ext_discount_amt IS NULL OR ws.ws_ext_discount_amt >= 0
    )
), HighProfitItems AS (
  SELECT
    rs.ws_item_sk,
  ...
  FROM web_sales AS ws
  JOIN HighProfitItems AS hpi
    ON ws.ws_item_sk = hpi.ws_item_sk
  GROUP BY
    ws.ws_item_sk
)
SELECT
  ...
  CASE
    WHEN NOT ss.avg_profit IS NULL
    THEN ROUND(ss.avg_profit, 2)
    ELSE (
      SELECT
        AVG(ws.ws_net_profit)
      FROM web_sales AS ws
      WHERE
        ws.ws_item_sk = ci.i_item_sk AND NOT ws.ws_net_profit IS NULL
    )
