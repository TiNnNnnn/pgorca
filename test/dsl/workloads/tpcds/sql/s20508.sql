WITH CustomerSales AS (
  ...
  WHERE
    NOT cd.cd_purchase_estimate IS NULL AND ws.ws_sales_price > 0
  GROUP BY
    c.c_customer_sk,
    c.c_first_name,
    c.c_last_name,
...
FULL OUTER JOIN StoresSummary AS ts
  ON tc.total_profit = ts.store_profit
WHERE
  (
    tc.total_profit > 1000 OR ts.store_profit IS NULL
  )
  AND (
    tc.gender_rank < 3 OR NOT ts.total_sales IS NULL
  )
ORDER BY
