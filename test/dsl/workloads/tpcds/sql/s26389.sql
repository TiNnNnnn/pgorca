WITH Address_Concat AS (
  ...
  SELECT
    ...
    d_moy,
    CASE
      WHEN d_moy IN (1, 2, 12)
      THEN 'Winter'
      WHEN d_moy IN (3, 4, 5)
      THEN 'Spring'
      WHEN d_moy IN (6, 7, 8)
      THEN 'Summer'
      WHEN d_moy IN (9, 10, 11)
      THEN 'Fall'
    END AS season
  FROM date_dim
  WHERE
...
SELECT
  ...
  dt.d_day_name,
  dt.season
FROM Address_Concat AS a
JOIN customer AS c
  ON a.ca_address_sk = c.c_current_addr_sk
JOIN Demographics_Filter AS d
  ON c.c_current_cdemo_sk = d.cd_demo_sk
JOIN Date_Filter AS dt
  ON dt.d_date_sk = c.c_first_sales_date_sk
ORDER BY
  a.full_address,
  d.cd_gender;
