WITH CustomerReturns AS (
  ...
  LEFT JOIN household_demographics AS hd
    ON c.c_customer_sk = hd.hd_demo_sk
)
SELECT
  ...
  CASE
    WHEN ss.avg_order_value IS NULL OR ss.avg_order_value = 0
    THEN 'No Order'
    WHEN ss.avg_order_value < 100
    THEN 'Low Value'
    ELSE 'High Value'
