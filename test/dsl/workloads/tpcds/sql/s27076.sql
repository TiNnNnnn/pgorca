WITH CustomerInfo AS (
  ...
  LEFT JOIN ReturnInfo AS ri
    ON ci.c_customer_sk = ri.sr_customer_sk
...
SELECT
  c.*,
  CASE
    WHEN total_sales > total_return_amount
    THEN 'Low Risk'
    WHEN total_returns > total_sales
    THEN 'High Risk'
    ELSE 'Moderate Risk'
  END AS customer_risk_level
FROM CombinedInfo AS c
WHERE
  c.ca_state = 'CA'
ORDER BY
