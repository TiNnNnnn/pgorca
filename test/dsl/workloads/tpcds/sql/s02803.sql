WITH CustomerReturns AS (
  ...
  LEFT JOIN CustomerReturns AS cr
    ON c.c_customer_sk = cr.cs_bill_customer_sk
  LEFT JOIN TopReturnReasons AS tr
    ON c.c_customer_sk = tr.wr_returning_customer_sk
  WHERE
    tr.wr_returning_customer_sk IS NULL
)
SELECT
  nrc.c_customer_id,
  nrc.total_sales,
  ...
  CASE
    WHEN nrc.total_sales > 10000
    THEN 'High Value'
    WHEN nrc.total_sales BETWEEN 5000 AND 10000
    THEN 'Medium Value'
    ELSE 'Low Value'
  END AS customer_value_segment
FROM NonReturningCustomers AS nrc
WHERE
  nrc.total_sales > 0
ORDER BY
  nrc.total_sales DESC;
