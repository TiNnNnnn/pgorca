...
SELECT
  ...
  CASE
    WHEN (
      sa.ws_quantity > id.total_quantity
    )
    THEN 'Exceeds Inventory'
    ELSE 'Within Inventory'
  END AS inventory_status,
  cs.customer_count AS segment_customer_count,
  cs.max_purchase_estimate
FROM customer AS c
JOIN sales_data AS sa
  ON c.c_customer_sk = sa.ws_order_number
LEFT JOIN inventory_data AS id
  ON sa.ws_item_sk = id.inv_item_sk
LEFT JOIN customer_segments AS cs
  ON c.c_current_cdemo_sk = cs.cd_demo_sk
WHERE
  sa.rn = 1 AND NOT c.c_birth_year IS NULL
ORDER BY
  c.c_last_name,
  c.c_first_name;
