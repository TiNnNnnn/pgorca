WITH StringBenchmark AS (
  SELECT
    ...
    SUBSTRING(c.c_email_address FROM STRPOS(c.c_email_address, '@') + 1) AS email_domain,
    REPLACE(c.c_email_address, '.', '-') AS modified_email,
    REGEXP_REPLACE(c.c_email_address, '^[^@]+', 'user') AS anonymized_email,
    CASE WHEN LENGTH(c.c_email_address) > 40 THEN 'Long Email' ELSE 'Short Email' END AS email_length_category,
...
SELECT
  ...
  sb.anonymized_email,
  sb.customer_rank
FROM StringBenchmark AS sb
JOIN StringCounts AS sc
  ON sb.full_name = sc.full_name
ORDER BY
