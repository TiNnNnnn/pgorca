WITH RECURSIVE sales_hierarchy AS (
  ...
  JOIN sales_hierarchy AS sh
    ON sh.c_customer_sk = s.ss_customer_sk
  WHERE
    sh.level < 2
  ...
  SELECT
    ...
    w.total_net_profit,
    w.total_orders
  FROM web_sales_summary AS w
  LEFT JOIN inventory_summary AS i
    ON w.ws_item_sk = i.inv_item_sk
)
SELECT
  sh.level,
  sh.path,
...
JOIN customer AS cs
  ON sh.c_customer_sk = cs.c_customer_sk
LEFT JOIN combined_sales AS cs_totals
  ON cs_totals.ws_item_sk = cs.c_current_cdemo_sk
WHERE
  sh.level <= 1
  AND (
    NOT cs.c_birth_year IS NULL OR NOT cs.c_email_address IS NULL
  )
GROUP BY
  sh.level,
  sh.path,
