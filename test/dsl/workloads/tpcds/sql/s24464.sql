WITH RecentSales AS (
  ...
  SELECT
    ...
    CASE
      WHEN COUNT(sr.sr_ticket_number) = 0
      THEN NULL
  ...
  SELECT
    ...
    (
      rs.total_sales - COALESCE(cr.total_returns, 0)
    ) AS net_sales,
    CASE
      WHEN COALESCE(cr.total_returns, 0) = 0
      THEN 0
      ELSE (
        cr.avg_return_amt / NULLIF(rs.total_sales, 0)
      ) * 100
    END AS return_rate_percentage
  FROM RecentSales AS rs
  LEFT JOIN CustomerReturns AS cr
    ON rs.ss_item_sk = cr.sr_item_sk
)
SELECT
  ad.ss_item_sk,
  ad.total_sales,
  ...
  CASE
    WHEN ad.return_rate_percentage > 20
    THEN 'High'
    WHEN ad.return_rate_percentage BETWEEN 10 AND 20
    THEN 'Moderate'
    ELSE 'Low'
  END AS return_rate_category
FROM AggregateData AS ad
WHERE
  NOT ad.return_rate_percentage IS NULL
ORDER BY
  ad.net_sales DESC
LIMIT 100;
