...
SELECT
  ...
  CASE
    WHEN s.AvgOrderValue IS NULL
    THEN 'No Sales'
    WHEN s.AvgOrderValue < 100
    THEN 'Low'
    ELSE 'High'
  END AS SalesCategory
FROM SalesWithRanking AS s
LEFT JOIN TopCustomers AS tc
  ON tc.CustomerTotalSpent BETWEEN 500 AND 10000
WHERE
  s.SalesRank <= 5 OR NOT tc.c_customer_id IS NULL
ORDER BY
  s.TotalSales DESC,
  TopCustomer DESC;
