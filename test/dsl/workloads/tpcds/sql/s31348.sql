...
SELECT
  ...
  CASE
    WHEN cj.customer_total_profit > 1000
    THEN 'High Value'
    WHEN cj.customer_total_profit BETWEEN 500 AND 1000
    THEN 'Medium Value'
    ELSE 'Low Value'
  END AS customer_value_category
FROM CustomerJoin AS cj
LEFT JOIN (
  SELECT
    hd_demo_sk,
  ...
  ON cj.c_customer_sk = avg_income.hd_demo_sk
ORDER BY
  cj.customer_total_profit DESC
LIMIT 100;
