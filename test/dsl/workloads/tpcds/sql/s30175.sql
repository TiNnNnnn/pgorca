WITH RECURSIVE sales_totals AS (
  ...
  JOIN ship_mode AS sm
    ON ws_ship_mode_sk = sm.sm_ship_mode_sk
  GROUP BY
    ws_ship_customer_sk,
    sm.sm_type
...
LEFT JOIN latest_shipping AS lt
  ON cs.c_customer_sk = lt.ws_ship_customer_sk
LEFT JOIN sales_totals AS st
  ON cs.c_customer_sk = st.ws_item_sk
LEFT JOIN inventory_summary AS inv
  ON cs.c_customer_sk = inv.inv_item_sk
WHERE
  cs.web_order_count > 5 AND (
    lt.order_count IS NULL OR lt.order_count > 2
  )
ORDER BY
  cs.c_last_name,
  cs.c_first_name;
