WITH RankedCustomers AS (
  ...
  WHERE
    NOT cd.cd_purchase_estimate IS NULL
), SalesData AS (
  SELECT
    ws.ws_item_sk,
  ...
  SELECT
    CASE
      WHEN ws.ws_sales_price - (
        ws.ws_ext_discount_amt / NULLIF(ws.ws_quantity, 0)
      ) > 0
      THEN (
        ws.ws_sales_price - (
          ws.ws_ext_discount_amt / NULLIF(ws.ws_quantity, 0)
  ...
  LEFT JOIN PromotionalSales AS ps
    ON sd.ws_item_sk = ps.net_price
)
SELECT
  rc.c_first_name,
  rc.c_last_name,
  ...
  CASE
    WHEN cs.avg_sales_per_item > (
      SELECT
        AVG(avg_sales_per_item)
      FROM CombinedSales
    )
    THEN 'Above Average'
    ELSE 'Below Average'
  END AS sales_performance
FROM CombinedSales AS cs
JOIN RankedCustomers AS rc
  ON cs.ws_item_sk = rc.c_customer_sk
WHERE
  rc.rnk <= 10
ORDER BY
