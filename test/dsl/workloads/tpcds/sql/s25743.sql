WITH EnhancedCustomerInfo AS (
  ...
  SELECT
    ...
    s.total_orders,
    s.total_revenue,
    CASE WHEN s.total_orders IS NULL THEN 'No Orders' ELSE 'Active Customer' END AS customer_status
  FROM EnhancedCustomerInfo AS e
  LEFT JOIN SalesSummary AS s
    ON e.c_customer_sk = s.ws_bill_customer_sk
)
SELECT
  *,
  CASE
    ...
    WHEN total_revenue BETWEEN 500 AND 1000
    THEN 'Medium Value Customer'
    ELSE 'Low Value Customer'
  END AS customer_value
FROM CustomerBenchmarking
ORDER BY
  total_revenue DESC,
  full_name;
