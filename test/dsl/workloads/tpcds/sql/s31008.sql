WITH RECURSIVE CTE_Sales AS (
  ...
  SELECT
    ...
    sales.total_sales
  FROM CTE_Sales AS sales
  JOIN item
    ON sales.ws_item_sk = item.i_item_sk
  WHERE
    sales.rank <= 10
), Customer_Summary AS (
  SELECT
  ...
  SELECT
    ...
    cu.avg_sales_price,
    CASE
      WHEN cu.total_net_profit > 1000
      THEN 'Gold'
      WHEN cu.total_net_profit BETWEEN 500 AND 1000
      THEN 'Silver'
      ELSE 'Bronze'
...
SELECT
  ...
  h.total_net_profit
FROM Top_Sales AS t
LEFT JOIN High_Value_Customers AS h
  ON h.total_purchases > 5
ORDER BY
  t.total_sales DESC,
