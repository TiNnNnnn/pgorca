WITH ranked_customers AS (
  ...
  WHERE
    NOT cd.cd_credit_rating IS NULL
  ...
  LEFT JOIN web_sales AS ws
    ON rc.c_customer_sk = ws.ws_bill_customer_sk
  WHERE
    rc.purchase_rank <= 5
  GROUP BY
    rc.c_customer_sk,
  ...
  LEFT JOIN customer_location AS cl
    ON hvc.c_customer_sk = cl.c_customer_sk
  WHERE
    cl.city_rank = 1
)
SELECT
...
WHERE
  NOT cwl.total_spent IS NULL
ORDER BY
  cwl.total_spent DESC
LIMIT 50;
