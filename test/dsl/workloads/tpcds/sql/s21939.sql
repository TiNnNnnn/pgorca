WITH RECURSIVE sales_data AS (
  ...
  FROM web_sales
  WHERE
    NOT ws_sales_price IS NULL AND ws_sales_price > 0
), sales_summary AS (
  SELECT
    sd.ws_item_sk,
  ...
  LEFT JOIN address_info AS ai
    ON ss.ws_item_sk = ai.c_customer_sk
  WHERE
    ss.total_sales > 1000
  ORDER BY
    ss.total_sales DESC
...
SELECT
  ...
  ts.ca_city,
  CASE WHEN ts.ca_state IS NULL THEN 'Unknown State' ELSE ts.ca_state END AS state
FROM top_sales AS ts
RIGHT JOIN warehouse AS w
  ON w.w_warehouse_sk = ts.ws_item_sk
ORDER BY
  ts.total_sales DESC,
  w.w_warehouse_name;
