...
SELECT
  ...
  CASE
    WHEN ic.available_stock IS NULL
    THEN 'No Stock Info'
    ELSE CASE
      WHEN ic.available_stock < 10
      THEN 'Low Stock'
      WHEN ic.available_stock BETWEEN 10 AND 50
      THEN 'Moderate Stock'
      ELSE 'High Stock'
    END
  END AS stock_status
...
LEFT JOIN sales_summary AS ss
  ON it.i_item_sk = ss.ws_item_sk
LEFT JOIN top_sales AS ts
  ON it.i_item_sk = ts.ss_item_sk
LEFT JOIN inventory_check AS ic
  ON it.i_item_sk = ic.inv_item_sk
WHERE
  (
    ss.sales_rank <= 10 OR NOT ts.total_quantity IS NULL
  )
  AND (
    NOT it.i_current_price IS NULL OR it.i_current_price > 0
  )
ORDER BY
