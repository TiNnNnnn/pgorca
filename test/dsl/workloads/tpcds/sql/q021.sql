/* end query 19 in stream 0 using template query58.tpl */ /* start query 20 in stream 0 using template query64.tpl */
WITH cs_ui AS (
  SELECT
    cs_item_sk,
  ...
  WHERE
    ...
    AND i_color IN ('blush', 'slate', 'sky', 'bisque', 'midnight', 'lime')
    AND i_current_price BETWEEN 53 AND 53 + 10
    AND i_current_price BETWEEN 53 + 1 AND 53 + 15
  GROUP BY
    i_product_name,
    i_item_sk,
    s_store_name,
