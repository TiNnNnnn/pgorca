/* end query 97 in stream 0 using template query61.tpl */ /* start query 98 in stream 0 using template query5.tpl */
WITH ssr AS (
  SELECT
    s_store_id,
  ...
  WHERE
    date_sk = d_date_sk
    AND d_date BETWEEN CAST('2000-08-12' AS DATE) AND (
      CAST('2000-08-12' AS DATE) + INTERVAL '14 DAY'
    )
