WITH CustomerReturns AS (
  ...
  FROM CustomerWithDemographics AS cd
  JOIN CustomerReturns AS cr
    ON cd.c_customer_sk = cr.sr_customer_sk
  WHERE
    cr.total_return_count > 5
  ORDER BY
    cr.total_returns_amt DESC
  LIMIT 100
)
SELECT
  tc.c_customer_sk,
...
JOIN CustomerDemographics AS cd
  ON tc.c_customer_sk = cd.cd_demo_sk
WHERE
  cd.cd_purchase_estimate > 500
ORDER BY
  tc.total_returns_amt DESC,
  tc.c_last_name ASC
LIMIT 50;
