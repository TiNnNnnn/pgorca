WITH RankedSales AS (
  ...
  FROM web_sales
  WHERE
    ws_sold_date_sk BETWEEN 2451916 AND 2451980
), CustomerStats AS (
  SELECT
    cd_gender,
  ...
  FROM store_returns
  WHERE
    sr_returned_date_sk BETWEEN 2451916 AND 2451980
), InventoryStats AS (
  SELECT
    inv_item_sk,
...
LEFT JOIN InventoryStats AS rk
  ON rk.inv_item_sk IN (
    SELECT
      ws_item_sk
    FROM RankedSales
    WHERE
...
LEFT JOIN TotalReturns AS tr
  ON 1 = 1
WHERE
  cs.total_customers > 10
ORDER BY
  cs.cd_gender,
