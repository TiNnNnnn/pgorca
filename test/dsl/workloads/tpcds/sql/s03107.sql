WITH RECURSIVE Revenue_CTE AS (
  ...
  SELECT
    ...
    SUM(CASE WHEN NOT ws.ws_ship_mode_sk IS NULL THEN ws.ws_quantity ELSE 0 END) AS shipped_quantity
  FROM customer AS c
...
LEFT JOIN Revenue_CTE AS rc
  ON cs.c_customer_sk = rc.ws_item_sk
WHERE
  cs.total_orders > 0 AND NOT rc.total_revenue IS NULL
ORDER BY
  cs.total_spent DESC,
