...
LEFT JOIN WeeklySales AS ws
  ON c.c_customer_sk = ws.ws_bill_customer_sk
LEFT JOIN InventoryLevels AS il
  ON il.inv_warehouse_sk = (
    SELECT
      w.w_warehouse_sk
    FROM warehouse AS w
    LIMIT 1
  )
LEFT JOIN PromotionStats AS p
  ON p.p_promo_sk = (
    SELECT
      MIN(promo.p_promo_sk)
    FROM promotion AS promo
  )
WHERE
  (
    cd.cd_gender = 'F' AND c.c_birth_month = 5
  )
  OR (
