WITH CustomerSales AS (
  ...
  WHERE
    NOT c.c_current_cdemo_sk IS NULL
  GROUP BY
    c.c_customer_sk,
    c.c_first_name,
    c.c_last_name,
...
SELECT
  ...
  p.total_orders,
  p.average_price,
  CASE WHEN tc.total_profit IS NULL THEN 'No sales' ELSE 'Sales recorded' END AS sales_status
FROM TopCustomers AS tc
LEFT JOIN ProductsSold AS p
  ON tc.c_customer_sk = p.ws_item_sk
ORDER BY
