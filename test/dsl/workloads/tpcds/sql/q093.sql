/* end query 89 in stream 0 using template query79.tpl */ /* start query 90 in stream 0 using template query18.tpl */
SELECT
  i_item_id,
  ca_country,
  ca_state,
...
WHERE
  ...
  AND c_birth_month IN (10, 11, 7, 9, 6, 4)
  AND d_year = 2000
  AND ca_state IN ('MS', 'OK', 'WY', 'IN', 'TX', 'PA', 'IL')
GROUP BY
ROLLUP (
  i_item_id,
  ca_country,
