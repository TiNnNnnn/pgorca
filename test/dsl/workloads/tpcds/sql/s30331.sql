WITH RECURSIVE sales_data AS (
  ...
  SELECT
    ...
    ci.return_count,
    CASE WHEN return_count > 0 THEN 'Returned' ELSE 'Not Returned' END AS return_status
  FROM customer_info AS ci
  LEFT JOIN top_sales AS r
    ON ci.c_customer_sk = r.ws_order_number
)
SELECT
  a.c_customer_sk,
  a.c_first_name,
  ...
  a.return_status,
  CASE
    WHEN a.return_count IS NULL
    THEN 'No Returns'
    ELSE CONCAT(a.return_count, ' Returns')
  END AS return_details
FROM annual_return_stat AS a
ORDER BY
  a.total_order_sales DESC
LIMIT 10;
