...
JOIN call_center AS ce
  ON ws.ws_ship_customer_sk = ce.cc_call_center_sk
JOIN date_dim AS d
  ON ws.ws_sold_date_sk = d.d_date_sk
LEFT JOIN store_returns AS sr
  ON c.c_customer_sk = sr.sr_customer_sk
LEFT JOIN inventory AS inv
  ON ws.ws_item_sk = inv.inv_item_sk AND inv.inv_warehouse_sk = ws.ws_warehouse_sk
LEFT JOIN promotion AS p
  ON ws.ws_promo_sk = p.p_promo_sk
WHERE
  d.d_year = 2023 AND NOT c.c_current_cdemo_sk IS NULL
GROUP BY
  c.c_customer_id,
  c.c_first_name,
  c.c_last_name,
