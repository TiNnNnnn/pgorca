WITH RECURSIVE sales_data AS (
  ...
  SELECT
    c.c_customer_sk,
    CASE WHEN cd_marital_status = 'M' THEN 'Married' ELSE 'Single' END AS marital_status,
...
SELECT
  ...
  EXISTS(
    ...
    WHERE
      sd2.profit_rank = 1 AND sd2.total_net_profit > 10000
  ) AS high_profit_item_exists
FROM customer_info AS ci
LEFT JOIN sales_data AS sd
  ON ci.c_customer_sk = sd.ws_item_sk
LEFT JOIN time_analysis AS ta
  ON ci.credit_rank <= ta.time_rank
GROUP BY
  ci.marital_status
HAVING
