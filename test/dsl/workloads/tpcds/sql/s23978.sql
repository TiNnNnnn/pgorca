...
LEFT JOIN InventoryAnalysis AS ia
  ON ia.inv_item_sk = rc.c_customer_sk
FULL OUTER JOIN SalesData AS sd
  ON sd.ws_item_sk = rc.c_customer_sk
WHERE
  rc.rn <= 5
  AND (
    rc.cd_marital_status = 'M' OR NOT rc.cd_purchase_estimate IS NULL
  )
  AND (
