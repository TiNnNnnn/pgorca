...
SELECT
  ...
  s.store_order_count,
  CASE
    WHEN s.total_web_sales > s.total_catalog_sales
    AND s.total_web_sales > s.total_store_sales
    THEN 'Web Sales Leading'
    WHEN s.total_catalog_sales > s.total_web_sales
    AND s.total_catalog_sales > s.total_store_sales
    THEN 'Catalog Sales Leading'
    WHEN s.total_store_sales > s.total_web_sales
    AND s.total_store_sales > s.total_catalog_sales
    THEN 'Store Sales Leading'
    ELSE 'Sales Equal'
