WITH RankedWebSales AS (
  ...
  FROM RankedWebSales AS rws
  JOIN item AS i
    ON rws.ws_item_sk = i.i_item_sk
  JOIN web_sales AS ws
    ON rws.ws_order_number = ws.ws_order_number AND rws.ws_item_sk = ws.ws_item_sk
  GROUP BY
    i.i_brand
...
FROM AggregateSales AS ag
LEFT JOIN StoreReturnsWithReasons AS srr
  ON ag.i_brand = srr.r_reason_desc
GROUP BY
  ag.i_brand,
  ag.total_quantity,
  ag.avg_net_profit
