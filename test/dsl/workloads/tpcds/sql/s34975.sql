...
FROM customer AS ia
LEFT JOIN ItemSales AS isales
  ON isales.i_item_sk IN (
    SELECT DISTINCT
      ws_item_sk
    FROM web_sales
    WHERE
...
LEFT JOIN promotion AS ip
  ON ip.p_item_sk = isales.i_item_sk
LEFT JOIN date_dim AS id
  ON id.d_date_sk = (
    SELECT
      MAX(ws_sold_date_sk)
    FROM web_sales
