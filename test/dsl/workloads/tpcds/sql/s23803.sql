WITH RankedSales AS (
  SELECT
    ...
    CASE WHEN AVG(COALESCE(sst.s_quantity, 0)) = 0 THEN 'No Sales' ELSE 'Sales Made' END AS sales_status
  ...
  LEFT JOIN (
    SELECT
      ss_item_sk,
      SUM(ss_quantity) AS s_quantity
    FROM store_sales
    WHERE
      ss_sold_date_sk > 20200101
    GROUP BY
      ss_item_sk
  ) AS sst
    ON ws.ws_item_sk = sst.ss_item_sk
  GROUP BY
    ws.ws_item_sk,
    ws.ws_order_number
), FilteredSales AS (
  ...
  JOIN item
    ON rs.ws_item_sk = item.i_item_sk
  WHERE
    rs.sales_rank <= 5
    AND (
...
SELECT
  ...
  cs.cumulative_sales,
  CASE
    WHEN cs.cumulative_sales IS NULL
    THEN 'Zero Cumulative'
    ELSE 'Cumulative Available'
  END AS cumulative_status
FROM CumulatedSales AS cs
WHERE
  NOT cs.total_sales IS NULL
ORDER BY
