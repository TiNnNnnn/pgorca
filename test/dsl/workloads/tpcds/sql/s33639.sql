WITH RecursiveSales AS (
  ...
  SELECT
    ...
    rs.order_count,
    CASE
      WHEN i.i_current_price > 50
      THEN 'Premium'
      WHEN i.i_current_price BETWEEN 20 AND 50
      THEN 'Standard'
      ELSE 'Budget'
    END AS price_category
  FROM RecursiveSales AS rs
  JOIN item AS i
  ...
  SELECT
    ...
    CASE WHEN cd.cd_purchase_estimate > 1000 THEN 'High Value' ELSE 'Low Value' END AS value_segment
  FROM customer AS c
  JOIN customer_demographics AS cd
    ON c.c_current_cdemo_sk = cd.cd_demo_sk
  LEFT JOIN CustomerReturns AS rp
    ON c.c_customer_sk = rp.sr_customer_sk
  GROUP BY
    cd.cd_gender,
    cd.cd_marital_status,
    rp.total_returns,
...
LEFT JOIN TotalPerformance AS tp
  ON cd.customer_count > 0
GROUP BY
  cd.cd_gender,
  cd.cd_marital_status
HAVING
