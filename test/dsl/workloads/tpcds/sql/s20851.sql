WITH RankedSales AS (
  ...
  WHERE
    r.profit_rank = 1 OR (
      r.order_count > 5 AND NOT r.total_profit IS NULL
    )
  ...
  SELECT
    ...
    ca.zip_code,
    CASE
      WHEN hvc.customer_rank <= 10
      THEN 'Top Tier'
      WHEN hvc.customer_rank BETWEEN 11 AND 50
      THEN 'Mid Tier'
      ELSE 'Low Tier'
    END AS customer_tier
  FROM HighValueCustomers AS hvc
  JOIN CustomerAddresses AS ca
    ON hvc.c_customer_id = ca.c_customer_id
)
SELECT
  f.*,
  CASE
    WHEN f.total_profit IS NULL
    THEN 'No Profit Recorded'
    ELSE CONCAT('Profit: $', ROUND(f.total_profit, 2))
  END AS profit_statement
FROM FinalReport AS f
WHERE
  NOT f.ca_city IS NULL
  AND (
    f.ca_state IN ('NY', 'CA') OR f.zip_code = 'UNKNOWN'
  )
ORDER BY
  f.total_profit DESC,
  f.c_customer_id
