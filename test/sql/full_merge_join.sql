...
CREATE FUNCTION disable_xform(text) RETURNS text
    AS 'pg_orca', 'DisableXform' LANGUAGE C STRICT;
CREATE FUNCTION enable_xform(text) RETURNS text
    AS 'pg_orca', 'EnableXform' LANGUAGE C STRICT;
...
drop table mj_cross;
drop table mj_part;
drop function disable_xform(text);
drop function enable_xform(text);
