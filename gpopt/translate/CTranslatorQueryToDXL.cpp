...
#define expression_tree_walker(n,w,c) \
	expression_tree_walker_impl(n,(bool(*)(Node*,void*))w,c)
#define query_tree_walker(q,w,c,f) \
	query_tree_walker_impl(q,(bool(*)(Node*,void*))w,c,f)
#define expression_tree_mutator(n,m,c) \
	expression_tree_mutator_impl(n,(Node*(*)(Node*,void*))m,c)
#define query_tree_mutator(q,m,c,f) \
	query_tree_mutator_impl(q,(Node*(*)(Node*,void*))m,c,f)
#define query_or_expression_tree_mutator(n,m,c,f) \
	query_or_expression_tree_mutator_impl(n,(Node*(*)(Node*,void*))m,c,f)
#include "utils/guc.h"
#include "utils/rel.h"
...
#endif
#ifndef PARENTSTMTTYPE_NONE
#define PARENTSTMTTYPE_NONE 0
#endif
#ifndef RTE_VOID
#define RTE_VOID RTE_JOIN
#endif
...
CTranslatorQueryToDXL::CheckSupportedCmdType(Query *query)
{
	...
	if (CMD_SELECT == query->commandType)
	{
		...
		/* parentStmtType / PARENTSTMTTYPE_* are GPDB-only Query fields.
		 * In PG18, all SELECT statements are treated as regular selects. */
...
CTranslatorQueryToDXL::TranslateQueryToDXL()
{
	...
	{
		case CMD_SELECT:
			/* parentStmtType is GPDB-only; always regular select in PG18 */
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	if (!optimizer_enable_dml)
	{
		...
		/* gp_random_insert_segments is GPDB MPP-only */
...
CTranslatorQueryToDXL::TranslateCTASToDXL()
{
	...
	/* intoPolicy is GPDB-only; in PG18, CTAS tables have no distribution policy */
	{
		GpdbEreport(
...
CTranslatorQueryToDXL::NoteDistributionPolicyOpclasses(const RangeTblEntry *rte)
{
	...
	{
		/* rd_cdbpolicy not in PG18 — distribution key opclass tracking skipped */
		(void) gpdb::GetRelation(rte->relid);  /* lock the relation */
	}
}
